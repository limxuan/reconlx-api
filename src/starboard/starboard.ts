...
export class StarboardClient {
    ...
    private generateEdit(starCount: number, message: Message): BaseMessageOptions {
        return {
    ...
    public async listener(reaction: MessageReaction) {
        ...
        if (reaction.emoji.name !== "⭐" || reaction.count < this.getData(guildId!)?.options.starCount) return;

        const data = this.cache.get(guildId!) || [];
        const starboardChannel = this.client.channels.cache.get(this.guilds.find((x) => x.id === guildId)?.options.starboardChannel) as TextChannel;
